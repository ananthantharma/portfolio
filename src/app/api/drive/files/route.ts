...
import { ensureFolder, getDriveClient, getOAuth2Client } from '@/lib/googleDrive';
...
export async function POST(req: Request) {
  try {
    ...
    if (contentType.includes('application/json')) {
      ...
      const oauth2Client = getOAuth2Client(session.accessToken, session.refreshToken);
      const { token: freshAccessToken } = await oauth2Client.getAccessToken();

      const initiateRes = await fetch(initiateUrl, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${freshAccessToken || session.accessToken}`,
