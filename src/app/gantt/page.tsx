  ...
  return (
    <div
      ...
      <div className="flex items-center space-x-3 overflow-hidden flex-1">
        {/* Drag Handle */}
        <div {...attributes} {...listeners} className="cursor-grab hover:text-[#6FBE44] text-slate-300 flex-shrink-0">
        ...
        <div
          ...
          style={{backgroundColor: categoryColors[task.category] || categoryColors['default']}}></div>
        <span
      ...
      <div className="flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity transform translate-x-2 group-hover:translate-x-0 duration-200 pl-2">
        ...
        <div onClick={() => onClick(task)} className="cursor-pointer p-1 hover:bg-slate-200 rounded">
...
export default function GanttPage() {
  ...
  const renderTimeScale = () => {
    ...
    for (let i = 0; i < numCols; i++) {
      ...
      const borderClass = `flex flex-col items-center justify-end border-r border-slate-200 h-full text-xs ${
        isWeekend && showDays ? 'bg-slate-50' : 'bg-white'
      }`;
    ...
    for (let i = 0; i < totalDays; i++) {
      ...
      cols.push(
        <div
          ...
          style={{width: COLUMN_WIDTH, minWidth: COLUMN_WIDTH}}></div>,
      );
    }
    return cols;
  };
      ...
      return (
        <div
          ...
          <div
            ...
            {width > 60 && (
              <span className="text-white text-xs font-medium px-2 whitespace-nowrap overflow-hidden text-ellipsis drop-shadow-md">
                {task.name}
              </span>
            )}
          </div>
              ...
              <input
                ...
                value={
                  viewMode === 'Custom'
                    ? columnWidth
                    : Math.round(
                        viewMode === 'Day' ? 50 : viewMode === 'Week' ? 30 : viewMode === 'Month' ? 15 : fitColumnWidth,
                      )
                }
                onChange={e => {
            ...
            <div className="bg-slate-100 p-1 rounded-lg flex text-sm shadow-inner hidden md:flex">
              <button
                ...
                className={`px-3 py-1 rounded-md transition-all duration-200 ${
                  viewMode === 'Day'
                    ? 'bg-white shadow-sm text-[#6FBE44] font-bold'
                    : 'text-slate-500 hover:text-[#404040]'
                }`}>
                Day
              </button>
              <button
          ...
          <div className="flex-1 flex flex-col overflow-hidden relative bg-slate-50" ref={containerRef}>
            ...
            <div className="flex-1 overflow-auto relative custom-scrollbar">
              <div className="relative min-h-full" style={{width: `${totalDays * COLUMN_WIDTH}px`}}>
                {/* Background Grid */}
                <div className="absolute inset-0 flex pointer-events-none">{renderGrid()}</div>

                {/* Bars Container */}
                <div className="absolute top-0 left-0 w-full pt-[0px]">{renderBars()}</div>
              </div>
            </div>
          </div>
        </div>
          ...
          {activeTask ? (
            <div className="group flex items-center justify-between px-4 h-[50px] bg-white border border-slate-200 shadow-xl opacity-90 rounded-lg">
              <div className="flex items-center space-x-3 overflow-hidden flex-1">
                ...
                <div
                  className="w-2.5 h-2.5 rounded-full flex-shrink-0 shadow-sm ring-2 ring-white"
                  style={{backgroundColor: categoryColors[activeTask.category] || categoryColors['default']}}></div>
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden transform scale-100 transition-all">
              <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                ...
                <button
                  ...
                  className="text-slate-400 hover:text-[#404040] transition-colors hover:rotate-90 duration-200">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <form onSubmit={handleSaveTask} className="p-6 space-y-5">
                ...
                <div className="flex items-center space-x-2 mb-4">
                  ...
                  />
                  <label htmlFor="isMilestone" className="text-sm font-bold text-[#404040]">
                    Mark as Milestone
                  </label>
                </div>
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden transform scale-100 transition-all border border-indigo-100">
              <div className="px-6 py-4 border-b border-indigo-50 flex justify-between items-center bg-gradient-to-r from-indigo-50 to-purple-50">
                ...
                <button
                  ...
                  className="text-slate-400 hover:text-indigo-600 transition-colors hover:rotate-90 duration-200">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <form onSubmit={handleRunAI} className="p-6">
                <p className="text-sm text-slate-600 mb-4">
                  Describe how you want to modify your Gantt chart. You can add tasks, shift dates, or change
                  categories.
                </p>
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
              ...
              <div className="p-6">
                {savedCharts.length === 0 ? (
                  <div className="text-center py-8 text-slate-400 text-sm">No saved projects found.</div>
                ) : (
                  <div className="space-y-2 max-h-60 overflow-y-auto custom-scrollbar">
                    {savedCharts.map(chart => (
                      <div
                        key={chart._id}
                        className="flex items-center justify-between p-3 border border-slate-100 rounded-xl hover:bg-slate-50 hover:border-[#6FBE44]/30 transition-all group cursor-pointer"
                        onClick={() => handleLoadChart(chart._id)}>
                        <div>
                          <div className="font-bold text-[#404040] group-hover:text-[#6FBE44] transition-colors">
                            {chart.name}
                          </div>
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden transform scale-100 transition-all">
              ...
              <div className="p-6">
                ...
                <form
                  ...
                  className="flex gap-2 mb-6 p-1.5 bg-slate-50 rounded-xl border border-slate-100 focus-within:ring-2 focus-within:ring-[#6FBE44]/20 focus-within:border-[#6FBE44] transition-all">
