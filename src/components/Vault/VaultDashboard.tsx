...
const SidebarItem = memo(
  ({active, icon, label, onClick}: {active?: boolean; icon: React.ReactNode; label: string; onClick: () => void}) => (
    <button
    ...
    `}>
      ...
      `}>
          <div className="flex items-center gap-1">
            ...
            <ActionButton active={isCopiedPass} icon={passIcon} onClick={handleCopyPass} tooltip="Copy Password" />
...
export const VaultDashboard: React.FC<VaultDashboardProps> = memo(
  ({onAdd, onDelete, onSignOut, onUpdate, passwords, userEmail}) => {
    ...
    const searchIcon = useMemo(
      () => <Search className="text-zinc-500 group-focus-within:text-orange-500 transition-colors" size={16} />,
      [],
    );
      ...
      <div className="flex h-screen bg-[#121212] text-zinc-100 overflow-hidden font-sans">
        ...
        <aside className="w-64 flex flex-col border-r border-[#1E1E1E] bg-[#121212]">
          ...
          <nav className="flex-1 px-3 space-y-1">
            <SidebarItem active={activeTab === 'all'} icon={vaultIcon} label="All Items" onClick={handleTabAll} />
            <SidebarItem
