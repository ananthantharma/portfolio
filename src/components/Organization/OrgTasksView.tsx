...
// We reuse ToDoBoard from Notes which is robust
import ToDoBoard from '../Notes/ToDoBoard';
...
export default function OrgTasksView() {
  ...
  const handleStatusChange = async (id: string, newStatus: string | undefined) => {
    if (!newStatus) return;
  ...
  const handleReorder = async (_activeId: string, _overId: string) => {
    // This provides optimistic ui
    // The ToDoBoard handles the actual array order
    fetchTodos();
  };

  const handleEdit = (_todo: TodoItem) => {
  ...
  const handleToggleCompleteBoard = async (todo: TodoItem) => {
  ...
  const handleAddDays = async (id: string, days: number) => {
    ...
    if (!todo) return;
    const newDueDate = new Date(todo.dueDate || Date.now());
    newDueDate.setDate(newDueDate.getDate() + days);

    setTodos(prev => prev.map(t => (t._id === id ? {...t, dueDate: newDueDate.toISOString()} : t)));
    try {
      await axios.put(`/api/todos/${id}`, {dueDate: newDueDate.toISOString()});
    } catch (error) {
      ...
      {viewMode === 'kanban' && !loading && !error && (
        <div className="flex-1 overflow-x-auto p-6 bg-slate-100/50">
          <ToDoBoard
            ...
            onToggleComplete={handleToggleCompleteBoard as any}
      ...
      <div className={`flex-1 overflow-y-auto p-6 ${viewMode === 'kanban' ? 'hidden' : ''}`}>
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <button onClick={fetchTodos} className="mt-3 text-indigo-600 text-xs hover:underline">
              Try again
            </button>
          </div>
        ) : filtered.length === 0 ? (
