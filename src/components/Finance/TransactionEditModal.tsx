...
const TransactionEditModal: React.FC<TransactionEditModalProps> = React.memo(
  ({isOpen, onClose, onSave, transaction}) => {
    ...
    const filteredCategories = useMemo(() => {
      return TRANSACTION_CATEGORIES.filter(c => c.toLowerCase().includes(searchTerm.toLowerCase()));
    }, [searchTerm]);
              ...
              <Transition.Child
                ...
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  <div className="mb-6">
                    ...
                    <p className="text-sm font-bold text-slate-800 mt-1">${transaction?.amount.toFixed(2)}</p>
                  </div>

                  <div className="mb-6">
                    <label className="block text-sm font-medium text-slate-700 mb-2">Select a category</label>
                      ...
                      {filteredCategories.map(cat => {
                        ...
                        return (
                          <button
                            ...
                            type="button">
                            <div
                              className={`mb-2 flex h-10 w-10 items-center justify-center rounded-full text-2xl ${
                                isSelected ? 'bg-white shadow-sm' : 'bg-slate-50 grayscale'
                              }`}>
