...
export default function InvestmentHistoryModal({
  ...
  const avgPrice = totalQty > 0 ? investments.reduce((sum, i) => sum + i.quantity * i.bookPrice, 0) / totalQty : 0;
                ...
                {investments.map(inv => (
                  <tr className="hover:bg-slate-50/50" key={inv._id}>
                    ...
                    <td className="px-4 py-3 text-right text-slate-600">
                      {formatCurrency(inv.quantity * inv.bookPrice)}
                    </td>
