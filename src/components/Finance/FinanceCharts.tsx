      ...
      <div className="h-96 w-full">
        <ResponsiveContainer height="100%" width="100%">
          <BarChart data={data} margin={CHART_MARGIN}>
            ...
            <XAxis axisLine={false} dataKey="name" stroke="#9ca3af" tick={TICK_STYLE} tickLine={false} />
            <YAxis
              axisLine={false}
              stroke="#9ca3af"
              tick={TICK_STYLE}
            ...
            {categories.map((category, index) => (
              <Bar dataKey={category} fill={COLORS[index % COLORS.length]} key={category} stackId="a" />
            ))}
          </BarChart>
        </ResponsiveContainer>
      </div>
