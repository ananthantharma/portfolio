...
const CSVUploader: React.FC<CSVUploaderProps> = React.memo(({lastUpdated, onUploadSuccess}) => {
      ...
      {message && (
        <div
          className={`text-xs px-2 py-1 rounded ${
            message.startsWith('Success') ? 'bg-green-900 text-green-200' : 'bg-red-900 text-red-200'
          }`}>
          {message}
        </div>
      )}
