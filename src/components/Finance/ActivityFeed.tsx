...
interface ActivityFeedProps {
  onBulkCategoryChange: (updates: {
    [id: string]: {category: string; amount?: number; type?: 'Income' | 'Expense'};
  }) => void;
  onCategoryChange: (id: string, category: string) => void;
  onClearAll: () => void;
  onDelete: (id: string) => void;
    ...
    <div className="group relative flex items-center justify-between rounded-xl bg-slate-50/50 p-3 transition-colors hover:bg-slate-100 border border-transparent hover:border-slate-200">
      <div className="flex items-center gap-4">
        <div className="relative">
          <button
            className={`flex h-10 w-10 cursor-pointer items-center justify-center rounded-full transition-transform hover:scale-105 active:scale-95 text-lg shadow-sm border ${
              t.type === 'Income'
...
const ActivityFeed: React.FC<ActivityFeedProps> = React.memo(
  ({onBulkCategoryChange, onCategoryChange, onClearAll, onDelete, onEdit, transactions}) => {
                ...
                {(['All', 'Income', 'Expense', 'Transfer'] as const).map(type => (
                  <button
                    className={`text-[10px] font-bold px-2 py-1 rounded-md transition-all ${
                      filterType === type ? 'bg-slate-800 text-white' : 'text-slate-500 hover:text-slate-700'
                    }`}
                    key={type}
        ...
        <div className="space-y-3 flex-1 overflow-y-auto custom-scrollbar pr-1">
          {filteredTransactions.length === 0 ? (
            ...
            filteredTransactions.map(t => (
              <ActivityItem key={t._id} onCategoryChange={onCategoryChange} onDelete={onDelete} onEdit={onEdit} t={t} />
            ))
          )}
        </div>
      </div>
