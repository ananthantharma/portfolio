import {Bot, Loader2, PlusCircle, Send, Trash2, User} from 'lucide-react';
...
export function OpenAIChatInterface({apiKey, onClearKey}: OpenAIChatInterfaceProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await getOpenAIChatResponse(apiKey, messages, userMessage, selectedModel);
    ...
    <div className="flex flex-col h-full bg-zinc-900 text-zinc-100">
      <header className="flex items-center justify-between px-6 py-4 bg-zinc-800 border-b border-zinc-700">
        ...
        <div className="flex items-center gap-3">
          <button
            className="text-sm text-zinc-400 hover:text-green-400 transition-colors flex items-center gap-2"
