...
#pfb-root {
  --dg:#0C2721;--green:#66CC00;--lgreen:#C7E995;--ink:#1b2420;
  ...
  color:var(--ink);background:#fff;position:fixed;inset:0;z-index:50;
}
#pfb-root header{background:var(--dg);color:#fff;padding:10px 16px;display:flex;align-items:center;gap:12px;flex-wrap:wrap;flex-shrink:0}
...
#pfb-root .back-btn{display:flex;align-items:center;gap:5px;background:rgba(102,204,0,.15);color:var(--lgreen);border:1px solid rgba(102,204,0,.3);border-radius:7px;padding:5px 10px;font-size:12px;font-weight:500;cursor:pointer;text-decoration:none;transition:background .15s;white-space:nowrap;font-family:inherit}
...
#pfb-root .bar .grp{display:flex;gap:4px;align-items:center;background:rgba(255,255,255,.08);padding:4px 6px;border-radius:7px}
#pfb-root .bar label{font-size:10.5px;color:#cfd6d1;text-transform:uppercase;letter-spacing:.4px}
#pfb-root button,#pfb-root select{font-family:inherit;font-size:12.5px;border:none;border-radius:6px;padding:6px 10px;cursor:pointer;background:#fff;color:var(--dg);font-weight:500}
...
#pfb-root .canvas-wrap{flex:1;overflow:auto;position:relative}
...
#pfb-root .node{position:absolute;border-radius:12px;border:2px solid;padding:12px 14px;cursor:grab;user-select:none;box-shadow:0 1px 3px rgba(0,0,0,.08);transition:box-shadow .12s}
#pfb-root .node:active{cursor:grabbing}
#pfb-root .node.sel{box-shadow:0 0 0 3px var(--green),0 4px 10px rgba(0,0,0,.15)}
#pfb-root .node.multi-sel{box-shadow:0 0 0 2px #66CC00,0 0 0 4px rgba(102,204,0,.25)}
...
#pfb-root .port{position:absolute;width:13px;height:13px;border-radius:50%;background:#fff;border:2px solid var(--green);opacity:0;transition:opacity .1s;cursor:crosshair;z-index:5}
...
#pfb-root .field input,#pfb-root .field textarea,#pfb-root .field select{width:100%;border:1px solid #d4d8d2;border-radius:7px;padding:7px 9px;font-size:13px;font-family:inherit;background:#fff;color:var(--ink)}
...
#pfb-root .zoom-ind{position:absolute;bottom:12px;left:12px;background:#fff;border:1px solid #e0e3df;border-radius:7px;padding:4px 10px;font-size:11.5px;color:#666;box-shadow:0 1px 4px rgba(0,0,0,.08)}
...
/* Legend */
#pfb-legend{position:absolute;background:#fff;border:2px solid #0C2721;border-radius:10px;padding:10px 14px;box-shadow:0 2px 10px rgba(0,0,0,.12);z-index:20;cursor:grab;user-select:none;min-width:130px}
#pfb-legend:active{cursor:grabbing}
#pfb-legend .leg-hdr{font-weight:700;font-size:11px;color:#0C2721;letter-spacing:.5px;display:flex;align-items:center;justify-content:space-between;max-height:0;overflow:hidden;opacity:0;transition:max-height .2s ease,opacity .15s ease,margin-bottom .2s ease;margin-bottom:0}
#pfb-legend:hover .leg-hdr{max-height:40px;opacity:1;margin-bottom:8px}
#pfb-legend .leg-lay-btn{background:#eee;color:#555;border:none;border-radius:4px;padding:2px 7px;font-size:10px;cursor:pointer;font-weight:600}
...
#pfb-legend .leg-lbl{font-size:11.5px;color:#333;outline:none;min-width:40px;border-bottom:1px solid transparent;transition:border-color .1s}
#pfb-legend .leg-lbl:hover{border-bottom-color:#ccc}
#pfb-legend .leg-lbl:focus{border-bottom-color:#66CC00}
#pfb-legend .leg-remove{background:none;border:none;color:#bbb;cursor:pointer;font-size:14px;padding:0 0 0 2px;line-height:1;flex-shrink:0;opacity:0;transition:opacity .1s}
...
.pfb-panel{position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background:#fff;border:1px solid #e0e3df;border-radius:14px;padding:22px;z-index:10000;min-width:300px;max-width:600px;width:95%;box-shadow:0 12px 40px rgba(0,0,0,.18)}
...
.pfb-panel textarea,.pfb-panel input[type=text]{width:100%;border:1px solid #d4d8d2;border-radius:8px;padding:10px;font-family:monospace;font-size:13px;resize:vertical;outline:none;color:#1b2420}
.pfb-panel textarea:focus,.pfb-panel input:focus{border-color:#66CC00}
.pfb-flow-item{display:flex;align-items:center;justify-content:space-between;padding:9px 11px;margin:4px 0;background:#f5f7f5;border-radius:8px;cursor:pointer;font-size:13px;font-family:-apple-system,sans-serif;transition:background .1s}
.pfb-flow-item:hover{background:#e8f5d4}
.pfb-flow-item span{flex:1;font-weight:500;color:#1b2420}
.pfb-flow-item .pfb-del{background:#fee2e2;color:#b91c1c;border:none;border-radius:5px;padding:3px 9px;font-size:11px;cursor:pointer;font-family:inherit}
.pfb-flow-item .pfb-del:hover{background:#fecaca}
.pfb-btn-row{display:flex;gap:8px;margin-top:12px}
.pfb-btn-primary{flex:1;background:#66CC00;color:#0C2721;border:none;border-radius:8px;padding:10px;font-size:13px;font-weight:600;cursor:pointer;font-family:-apple-system,sans-serif}
.pfb-btn-primary:hover{background:#7adb12}
.pfb-cancel{background:#eceae3;color:#444;border:none;border-radius:8px;padding:10px 16px;font-size:12.5px;cursor:pointer;font-family:-apple-system,sans-serif}
...
/* Hierarchy modal */
.hier-split{display:flex;gap:14px;margin-bottom:12px}
.hier-left{flex:0 0 200px;border:1px solid #e3e6e1;border-radius:8px;overflow-y:auto;max-height:280px;background:#fafafa}
.hier-left-hdr{font-size:10.5px;font-weight:600;color:#666;text-transform:uppercase;letter-spacing:.4px;padding:8px 10px 4px;border-bottom:1px solid #e3e6e1}
.hier-node-row{display:flex;align-items:center;gap:7px;padding:5px 10px;cursor:pointer;font-size:12px;color:#333;transition:background .1s;border-radius:5px;margin:2px 4px}
.hier-node-row:hover{background:#e8f5d4}
.hier-node-row.used{color:#aaa;font-style:italic}
.hier-node-swatch{width:10px;height:10px;border-radius:2px;flex-shrink:0;border:1px solid rgba(0,0,0,.15)}
...
<header>
  ...
  <div class="bar">
    ...
    <div class="grp">
      <label>Layout</label>
      <select id="dir"><option value="TB">Vertical ↓</option><option value="LR">Horizontal →</option></select>
      <select id="wrap" title="Boxes per row before wrapping">
        <option value="0">No limit</option><option value="2">Wrap 2</option><option value="3">Wrap 3</option>
        <option value="4" selected>Wrap 4</option><option value="5">Wrap 5</option><option value="6">Wrap 6</option>
        <option value="8">Wrap 8</option><option value="10">Wrap 10</option><option value="custom">Custom…</option>
      </select>
      <input id="wrapCustom" type="number" min="1" max="50" value="4" title="Custom wrap"
    ...
    <div class="grp">
      <button id="zoomOut">−</button><button id="zoomIn">+</button><button id="fit">Fit</button>
    </div>
    <div class="grp">
      <label>Canvas</label>
...
<div class="main">
  ...
  <aside class="side" id="side"><div id="inspector"></div></aside>
</div>
`;
...
export default function ProcessFlowBuilder() {
  ...
  useEffect(() => {
    ...
    type AppState = {
      nodes: Node[]; edges: Edge[]; dir: string; wrap: number;
      legend: LegendState; canvasBg: string;
    };

    // ── Constants ──────────────────────────────────────────────────────────
    const THEMES = [
      {k:'start',name:'Start / End'},{k:'blue',name:'Process Step'},
      {k:'green',name:'Approved'},{k:'grey',name:'Note / Neutral'},
      {k:'amber',name:'Caution'},{k:'purple',name:'Decision'},{k:'red',name:'Issue / Stop'},
    ];
    ...
    // FIX: explicit Map so loadState never has stale DOM lookups
    ...
    function makeNode(opts: Partial<Node>={}): Node {
      const base = {id:'n'+idc++,x:80,y:80,w:210,h:74,title:'New step',sub:'',theme:'blue',shape:'rect',badge:'',border:'',font:''};
      if (opts.id) {
        // Explicit ID provided (e.g. seed) — ensure idc stays ahead
        const num = +opts.id.replace(/\D/g,'');
        if (!isNaN(num) && num >= idc) idc = num+1;
        // Override base id with the given one
        base.id = opts.id;
      }
      const n: Node = {...base, ...opts, id: base.id};
      state.nodes.push(n);
      return n;
    }

    // ── Build / update node element ────────────────────────────────────────
    function nodeEl(n: Node): HTMLDivElement {
      ...
      const inSel = selNodes.has(n.id);
      el.className =
        'node t-'+n.theme+(n.shape==='pill'?' shape-pill':'')+
        (inSel && selNodes.size===1?' sel':inSel?' multi-sel':'');
      el.style.cssText =
      ...
      badge.textContent = n.badge||''; badge.style.display = n.badge?'block':'none';
    ...
    function render() {
      ...
      // Remove DOM for deleted nodes
      nodeEls.forEach((el,id)=>{
        if (!state.nodes.find(n=>n.id===id)){el.remove();nodeEls.delete(id);}
      });
      drawEdges();
      renderLegend();
      inspector();
    ...
    function parsePathPts(d: string):{x:number;y:number}[]{
      return (d.match(/[ML]\s*[\d.,-]+\s*[\d.,-]+/g)||[]).map(cmd=>{
        const parts=cmd.slice(1).trim().split(/[\s,]+/);
        return {x:+parts[0],y:+parts[1]};
      });
    }

    // ── Edge crossing bridge rendering ─────────────────────────────────────
    function drawBridges(pathData:{d:string;color:string;order:number}[]){
      const R=8; // bridge radius
      interface Crossing{x:number;y:number;underColor:string}
      const crossings:Crossing[]=[];

      for (let ai=0;ai<pathData.length;ai++){
        for (let bi=ai+1;bi<pathData.length;bi++){
          const ptA=parsePathPts(pathData[ai].d);
          const ptB=parsePathPts(pathData[bi].d);
          for (let i=0;i<ptA.length-1;i++){
            for (let j=0;j<ptB.length-1;j++){
              const a1=ptA[i],a2=ptA[i+1],b1=ptB[j],b2=ptB[j+1];
              const aH=Math.abs(a1.y-a2.y)<1;
              const bH=Math.abs(b1.y-b2.y)<1;
              if (aH===bH) continue; // both H or both V, no crossing
              const hSeg=aH?{x1:Math.min(a1.x,a2.x),x2:Math.max(a1.x,a2.x),y:a1.y}
                            :{x1:Math.min(b1.x,b2.x),x2:Math.max(b1.x,b2.x),y:b1.y};
              const vSeg=aH?{y1:Math.min(b1.y,b2.y),y2:Math.max(b1.y,b2.y),x:b1.x}
                            :{y1:Math.min(a1.y,a2.y),y2:Math.max(a1.y,a2.y),x:a1.x};
              if (vSeg.x>hSeg.x1+2&&vSeg.x<hSeg.x2-2&&hSeg.y>vSeg.y1+2&&hSeg.y<vSeg.y2-2){
                // "under" is the H segment's edge (the one drawn first = lower z)
                crossings.push({
                  x:vSeg.x,y:hSeg.y,
                  underColor: aH ? pathData[ai].color : pathData[bi].color,
                });
              }
            }
          }
      ...
      crossings.forEach(({x,y,underColor:_})=>{
        // White gap on the horizontal (under) line
        const gap=document.createElementNS('http://www.w3.org/2000/svg','rect');
        gap.setAttribute('x',String(x-R)); gap.setAttribute('y',String(y-4));
        gap.setAttribute('width',String(R*2)); gap.setAttribute('height','8');
        gap.setAttribute('fill','white'); gap.setAttribute('stroke','none');
        gap.style.pointerEvents='none';
        svg.appendChild(gap);
        // Small arc for the "over" (vertical) line
        const arc=document.createElementNS('http://www.w3.org/2000/svg','path');
        arc.setAttribute('d',`M${x},${y-R} A${R},${R} 0 0,1 ${x},${y+R}`);
        arc.setAttribute('fill','none'); arc.setAttribute('stroke','#5a6159');
        arc.setAttribute('stroke-width','2'); arc.style.pointerEvents='none';
        svg.appendChild(arc);
      });
    }
    ...
    function drawEdges(){
      ...
      usedColors.add('#66CC00');
      let defs='<defs>';
      usedColors.forEach(c=>{
        const id='arr-'+c.replace('#','');
        defs+=`<marker id="${id}" markerWidth="10" markerHeight="10" refX="8" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 z" fill="${c}"/></marker>`;
      });
      defs+='</defs>';
      svg.innerHTML=defs;

      const pathData:{d:string;color:string;order:number}[]=[];

      state.edges.forEach((e,order)=>{
        ...
        pth.setAttribute('marker-end',`url(#${markId})`);pth.setAttribute('class','vis');
        ...
        if (e.label){
          ...
          r.addEventListener('click',ev=>{ev.stopPropagation();selEdge=e.id;selNodes.clear();render();});
          const t=document.createElementNS('http://www.w3.org/2000/svg','text');
          t.textContent=e.label;t.setAttribute('x',String(mid.x));t.setAttribute('y',String(mid.y+4));
    ...
    function bindNode(el: HTMLDivElement, n: Node){
      el.addEventListener('mousedown', ev=>{
        ...
        if (tgt.classList.contains('port')){startEdge(n,tgt.dataset.d!,ev);return;}
        ev.stopPropagation();

        // Multi-select logic
        if (ev.shiftKey){
          if (selNodes.has(n.id)) selNodes.delete(n.id);
          else selNodes.add(n.id);
          selEdge=null;
          updateSelClasses();
          inspector();
        } else {
          if (!selNodes.has(n.id)){
            selNodes.clear();selNodes.add(n.id);selEdge=null;
            updateSelClasses();
            inspector();
          }
          // Drag all selected nodes together
          const sx=ev.clientX,sy=ev.clientY;
          const snapshots=[...selNodes].map(id=>{
            const nd=state.nodes.find(n=>n.id===id)!;
            return {nd,ox:nd.x,oy:nd.y};
          });
          let moved=false;
          function mv(e: MouseEvent){
            ...
            snapshots.forEach(({nd,ox,oy})=>{
              nd.x=Math.max(0,ox+dx);nd.y=Math.max(0,oy+dy);
    ...
    // ── Lasso selection ────────────────────────────────────────────────────
    function startLasso(startEv: MouseEvent){
      const r0=canvas.getBoundingClientRect();
      const sx=(startEv.clientX-r0.left)/zoom;
      const sy=(startEv.clientY-r0.top)/zoom;
      ...
      function mv(e: MouseEvent){
        ...
        lasso.style.left=Math.min(sx,cx)+'px';lasso.style.top=Math.min(sy,cy)+'px';
        lasso.style.width=Math.abs(cx-sx)+'px';lasso.style.height=Math.abs(cy-sy)+'px';
      }
      function up(e: MouseEvent){
        document.removeEventListener('mousemove',mv);document.removeEventListener('mouseup',up);
        const r=canvas.getBoundingClientRect();
        const cx=(e.clientX-r.left)/zoom,cy=(e.clientY-r.top)/zoom;
        const lx=Math.min(sx,cx),ly=Math.min(sy,cy),lw=Math.abs(cx-sx),lh=Math.abs(cy-sy);
        lasso.remove();
        if (lw>6||lh>6){
          if (!e.shiftKey) selNodes.clear();
          state.nodes.forEach(n=>{
            if (n.x+n.w>lx&&n.x<lx+lw&&n.y+n.h>ly&&n.y<ly+lh) selNodes.add(n.id);
          });
          selEdge=null;
          render();
        }
    ...
    // ── Canvas click → deselect or lasso ──────────────────────────────────
    cw.addEventListener('mousedown', e=>{
      const t=e.target as HTMLElement;
      if (t===cw||t===canvas||(t as unknown)===svg){
        if (!e.shiftKey){selNodes.clear();selEdge=null;updateSelClasses();inspector();}
        startLasso(e);
      }
    });
    ...
    function inspector(){
      ...
      if (selEdge){
        const e=state.edges.find(x=>x.id===selEdge);
      ...
      if (selNodes.size>1){
        ...
        ins.innerHTML=`<h2>${selNodes.size} boxes selected</h2><div class="pad">
          <div class="field"><label>Change colour for all</label><div class="swatches" id="mswrap"></div></div>
          <button class="del" id="mdelAll" style="width:100%;margin-top:6px">Delete all selected</button>
        </div>`;
        const sw=document.getElementById('mswrap') as HTMLElement;
        THEMES.forEach(t=>{
      ...
      // Single node
      const n=state.nodes.find(x=>x.id===[...selNodes][0]);
      if (!n){ins.innerHTML='';return;}
      ins.innerHTML=`
       <h2>Box</h2><div class="pad">
    ...
    function inspectEdge(e: Edge){
      ...
      ins.innerHTML=`<h2>Connection</h2><div class="pad">
        ...
        <div class="field"><label>Line colour</label>
          <div class="row2" style="align-items:center">
            <input type="color" id="e-color" value="${ec}" style="height:34px;padding:2px">
    ...
    function delNodeQuiet(id: string){
    ...
    function renderLegend(){
      ...
      if (!leg){
        leg=document.createElement('div');leg.id='pfb-legend';
        canvas.appendChild(leg);bindLegendDrag(leg);
      }
      const isH=state.legend.layout==='H';
      const hidden=state.legend.hidden||[];
      const hasHidden=hidden.length>0;
      leg.style.left=state.legend.x+'px';leg.style.top=state.legend.y+'px';
      const visibleT=THEMES.filter(t=>!hidden.includes(t.k));
      const items=visibleT.map(t=>{
      ...
      leg.innerHTML=`
        <div class="leg-hdr"><span>LEGEND</span>
          <div style="display:flex;gap:4px">
            <button class="leg-lay-btn${!isH?' active':''}" data-lay="V">V</button>
            <button class="leg-lay-btn${isH?' active':''}" data-lay="H">H</button>
            ${hasHidden?`<button class="leg-lay-btn" data-restore="1" title="Restore ${hidden.length} hidden">↺</button>`:''}
          </div>
        </div>
...
End"></textarea>
    ...
    function autoLayout(){
      ...
      const queue=[...roots],indc={...ind};let guard=0;
      while(queue.length&&guard++<5000){
        const id=queue.shift()!;
        (out[id]||[]).forEach(t=>{depth[t]=Math.max(depth[t]??0,(depth[id]??0)+1);if(--indc[t]===0)queue.push(t);});
      }
      ...
      let mainPos=padY;
      keys.forEach(lk=>{
        const grp=levels[lk];let maxMain=0;
        grp.forEach((n,i)=>{
    ...
    function applyChainWrap(dir: string,wrap: number){
      ...
      const colW=250,rowH=130,padX=60,padY=60;
      order.forEach((n,i)=>{
        const band=Math.floor(i/wrap),idx=i%wrap;
        const pos=band%2===0?idx:wrap-1-idx;
        if(dir==='TB'){n.x=padX+pos*colW;n.y=padY+band*rowH;}
        else{n.x=padX+band*colW*1.2;n.y=padY+pos*rowH;}
      });
    }

    // ── Zoom / fit ─────────────────────────────────────────────────────────
    function applyZoom(){canvas.style.transform=`scale(${zoom})`;(document.getElementById('zi') as HTMLElement).textContent=Math.round(zoom*100)+'%';}
    function fit(){
      ...
      zoom=Math.min(1,(cw.clientWidth-40)/(maxx-minx+60),(cw.clientHeight-40)/(maxy-miny+60));
    ...
    function exportPNG(){
      ...
      const x=c.getContext('2d')!;x.scale(2,2);x.fillStyle=state.canvasBg||'#fff';x.fillRect(0,0,W,H);
    ...
    (document.getElementById('addNode') as HTMLButtonElement).onclick=()=>{
      ...
      selNodes=new Set([n.id]);selEdge=null;
      render();
    ...
    (document.getElementById('legendBtn') as HTMLButtonElement).onclick=()=>{
      ...
      (document.getElementById('legendBtn') as HTMLButtonElement).style.background=state.legend.visible?'rgba(102,204,0,.35)':'';
      renderLegend();
    };
    ...
    (document.getElementById('clear') as HTMLButtonElement).onclick=()=>{
      if(confirm('Clear the whole canvas?')){
        ...
        selNodes.clear();selEdge=null;idc=1;edgeId=1;render();
      }
    };
