...
const TimelineBox: FC<TimelineBoxProps> = memo(({ title, items, side = 'left' }) => {
    return (
        <div className="flex h-full w-full flex-col overflow-hidden p-2 lg:p-4">
            ...
            <div className={`flex-1 overflow-y-auto pr-2 scrollbar-hide ${side === 'left' ? 'border-r border-white/10' : 'border-l border-white/10'}`}>
                    ...
                    {items.map((item, index) => (
                        ...
                        >
                            ...
                            <div className="flex-1">
                                ...
                                <h3 className="bg-gradient-to-r from-[#00C9FF] to-[#92FE9D] bg-clip-text text-lg font-bold text-transparent lg:text-xl">{item.title}</h3>
                            ...
                            {/* Connector Line */}
                            <div className={`absolute top-1/2 h-px w-4 bg-gradient-to-r from-cyan-400 to-transparent ${side === 'left' ? '-right-4' : '-left-4 rotate-180'}`} />
                        </div>
                    ))}
                </div>
            </div>
