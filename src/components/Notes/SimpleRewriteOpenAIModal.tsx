...
interface SimpleRewriteOpenAIModalProps {
    isOpen: boolean;
    onClose: () => void;
}

const SimpleRewriteOpenAIModal: React.FC<SimpleRewriteOpenAIModalProps> = memo(({ isOpen, onClose }) => {
...
`;
    ...
    const handleRewrite = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                setRewrittenText(`Error: ${data.details || data.error || 'Unknown error'}`);
            } else {
                                ...
                                <div className="flex flex-col gap-6 h-[500px]">
                                    ...
                                    <div className="flex-1 flex gap-4 h-full">
                                        ...
                                        <div className="flex-1 flex flex-col min-h-0">
                                            ...
                                            <textarea
                                                className="flex-1 p-4 bg-gray-50 rounded-lg border border-gray-300 text-sm text-gray-800 focus:ring-2 focus:ring-teal-500 focus:border-transparent resize-none leading-relaxed"
                                        ...
                                        <div className="flex flex-col justify-center items-center gap-2">
                                            <button
                                                className="rounded-full bg-teal-600 p-3 text-white shadow-lg hover:bg-teal-700 disabled:opacity-50 transition-all hover:scale-105 active:scale-95"
                                                disabled={isGenerating || !inputText.trim()}
                                                onClick={handleRewrite}
                                                title="Rewrite"
                                            >
                                                {isGenerating ? <ArrowPathIcon className="h-6 w-6 animate-spin" /> : <ArrowPathIcon className="h-6 w-6" />}
                                            </button>
                                        </div>
                                        ...
                                        <div className="flex-1 flex flex-col min-h-0">
                                            <div className="flex justify-between items-center mb-2">
                                                ...
                                                {rewrittenText && (
                                                    <button
                                                        className="text-xs flex items-center gap-1 text-teal-600 hover:text-teal-800 font-bold bg-teal-50 px-2 py-1 rounded-md"
                                                        onClick={copyToClipboard}>
                                                        <ClipboardDocumentIcon className="h-4 w-4" /> Copy
                                                    </button>
                                                )}
                                            </div>
                                            <div className="flex-1 p-4 bg-white rounded-lg border-2 border-teal-100 text-sm text-gray-800 shadow-inner overflow-y-auto relative">
                                                {isGenerating ? (
                                                    <div className="absolute inset-0 flex flex-col items-center justify-center text-teal-600 space-y-3 bg-white/80">
                                                        <ArrowPathIcon className="h-10 w-10 animate-spin" />
                                                        <span className="font-medium animate-pulse">Simplifying with OpenAI...</span>
                                                    </div>
                                                ) : (
...
SimpleRewriteOpenAIModal.displayName = 'SimpleRewriteOpenAIModal';
export default SimpleRewriteOpenAIModal;
