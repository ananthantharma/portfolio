...
const UnifiedAIChatModal: React.FC<UnifiedAIChatModalProps> = React.memo(
  ({isOpen, onClose, geminiApiKey, openaiApiKey}) => {
    ...
    const currentModel = useMemo(() => ALL_MODELS.find(m => m.id === selectedModel) || ALL_MODELS[0], [selectedModel]);
      ...
      async (sessionToSave: LocalSession) => {
        ...
        try {
          if (sessionToSave.dbId) {
            ...
            setSessions(prev => prev.map(s => (s.localId === sessionToSave.localId ? {...s, isDirty: false} : s)));
          } else {
            // Create new
            const res = await fetch('/api/ai-chat/sessions', {
              method: 'POST',
            ...
            if (res.ok) {
              const data = await res.json();
              setSessions(prev =>
                prev.map(s => (s.localId === sessionToSave.localId ? {...s, dbId: data.data._id, isDirty: false} : s)),
              );
    ...
    const deleteSavedSession = useCallback(async (e: React.MouseEvent, dbId: string) => {
      e.stopPropagation();
      try {
    ...
    const handleSubmit = useCallback(
      async (e: React.FormEvent) => {
        ...
        const userMessage: ChatMessage = {
          role: 'user',
          content:
            userMessageText || (currentAttachments.length > 0 ? `[Attached ${currentAttachments.length} file(s)]` : ''),
        ...
        try {
          ...
          const systemInstruction = sessionLocal?.systemInstruction || DEFAULT_SYSTEM_INSTRUCTION;
            ...
            <div className="flex-1 overflow-y-auto">
              {sessions.length > 0 && (
                <div className="p-2">
                  ...
                  {sessions.map(session => (
                    <div
                      className={`group flex items-center gap-2 px-3 py-2.5 rounded-lg cursor-pointer transition-all text-sm mb-0.5 ${
                        currentSessionId === session.localId
                      ...
                      <div
                        className={`w-2 h-2 rounded-full flex-shrink-0 ${
                          session.provider === 'gemini' ? 'bg-blue-400' : 'bg-emerald-400'
                        }`}
                      />
              ...
              <div className="p-2 border-t border-zinc-800/50">
                ...
                {isLoadingSessions ? (
                  ...
                  <p className="px-3 py-4 text-xs text-zinc-600 text-center">No saved chats yet</p>
                ) : (
                    ...
                    <div
                      ...
                      <div
                        className={`w-2 h-2 rounded-full flex-shrink-0 opacity-50 ${
                          saved.provider === 'gemini' ? 'bg-blue-400' : 'bg-emerald-400'
                        }`}
                      />
            ...
            <header className="flex items-center justify-between px-6 py-3.5 bg-zinc-800/50 border-b border-zinc-700/50 backdrop-blur-xl">
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <h2 className="text-sm font-semibold text-white">{currentSession?.title || 'AI Assistant'}</h2>
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
              {!currentSession || currentSession.messages.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-zinc-500 space-y-4">
                  {isLoadingMessages ? (
                    ...
                    <>
                      ...
                      <p className="text-lg font-medium text-zinc-400">What can I help you with?</p>
                      <p className="text-sm text-zinc-600">Select a model and start chatting</p>
                    </>
                  )}
                </div>
              ) : (
                currentSession.messages.map((msg, idx) => {
                  ...
                  return (
                    <div className={`flex gap-4 ${isUser ? 'justify-end' : 'justify-start'}`} key={idx}>
                      <div className={`flex gap-3 max-w-[80%] ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
                        <div
                          className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                            isUser ? 'bg-violet-600' : providerColor.avatarBg
                          }`}>
            ...
            <div className="p-4 bg-zinc-900/80 border-t border-zinc-800 backdrop-blur-xl">
              ...
              <form className="max-w-4xl mx-auto" onSubmit={handleSubmit}>
                  ...
                  <div className="flex gap-2 mb-3 flex-wrap">
                    {attachments.map((att, idx) => (
                      <div
                        ...
                        key={idx}>
                        {att.type === 'image' ? (
                          <img alt={att.name} className="h-8 w-8 rounded object-cover" src={att.content} />
                        ) : (
                ...
                <div className="relative flex items-end gap-2">
                  ...
                  <div className="relative flex-1">
                    <textarea
                      ...
                      onPaste={handlePaste}
                      placeholder={`Message ${
                        currentModel.provider === 'gemini' ? 'Gemini' : 'OpenAI'
                      }... (paste images here)`}
                      ref={textareaRef}
                      rows={1}
                      value={input}
                ...
                <div className="flex items-center justify-between mt-2 px-1">
                  ...
                  <span className="text-[10px] text-zinc-600">Shift+Enter for new line · Paste images with Ctrl+V</span>
                </div>
              </form>
            </div>
          </div>
