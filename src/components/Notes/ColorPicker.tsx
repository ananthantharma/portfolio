...
export const ColorPicker: React.FC<ColorPickerProps> = React.memo(({selectedColor, onSelectColor}) => {
  return (
    <div className="flex flex-wrap gap-2">
      {COLOR_PALETTE.map(color => (
        <button
          className={`h-6 w-6 rounded-full transition-transform hover:scale-110 focus:outline-none focus:ring-2 focus:ring-offset-2 ${
            selectedColor === color.value ? 'ring-2 ring-gray-400 ring-offset-1 scale-110' : ''
          }`}
          key={color.value}
