...
function SortableColumnHeader({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`relative flex items-center justify-between px-3 py-2.5 text-[11px] font-semibold uppercase tracking-wider select-none border-r border-gray-200 group
                ${column.id === 'name' ? 'sticky left-0 z-30' : ''}
                ${isDragging ? 'bg-blue-50' : 'bg-gray-50'}`}>
      <div className="flex items-center gap-1.5 overflow-hidden flex-1 min-w-0">
        {column.id !== 'name' && (
          <div {...attributes} {...listeners} className="cursor-grab hover:text-gray-600 text-gray-300 shrink-0">
        ...
        {editing ? (
          <input
            ref={inputRef}
            className="bg-white text-gray-800 text-[11px] font-semibold uppercase rounded px-1 py-0.5 border border-indigo-400 outline-none w-full"
            value={label}
          ...
          <span
            className="truncate text-gray-500 cursor-pointer hover:text-gray-800 transition-colors"
...
function AddColumnInline({ onAdd }: { onAdd: (col: ColumnDefinition) => void }) {
  ...
  const submit = () => {
    ...
    const newCol: ColumnDefinition = {
      ...
      align:
        type === 'currency' || type === 'number' ? 'right' : type === 'date' || type === 'status' ? 'center' : 'left',
    };
    if (type === 'status') {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        className="h-full px-4 py-2.5 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 transition-colors border-r border-gray-200"
        title="Add column">
        <PlusIcon className="w-4 h-4" />
      </button>
      {open && (
        <div className="absolute top-full left-0 mt-1 w-64 bg-white border border-gray-200 rounded-xl shadow-xl p-4 z-50 space-y-3">
          <input
            className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-800 placeholder-gray-400 outline-none focus:ring-2 focus:ring-indigo-400"
...
export default function TableApp() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (currentTableId) {
        ...
        const nameToUse =
          currentTableName === 'Untitled Table' ? prompt('Enter table name:', currentTableName) : currentTableName;
        if (!nameToUse) {
          setLoading(false);
          return;
        }
  ...
  const renderCell = (row: TableRow, col: ColumnDefinition, depth: number) => {
    ...
    if (col.id === 'name') {
      return (
        <div className="flex items-center gap-1.5">
          ...
          <input
            className="bg-transparent text-gray-800 text-sm outline-none w-full placeholder-gray-300 font-medium"
            value={value || ''}
    ...
    if (col.type === 'status') {
      ...
      return (
        <div className="relative w-full" data-status-dropdown>
          ...
          {isOpen && (
            <div
              className="absolute top-full left-1/2 -translate-x-1/2 mt-1 flex flex-col bg-white border border-gray-200 rounded-lg shadow-xl py-1 z-50 min-w-[130px]"
    ...
    if (col.type === 'date') {
      return (
        <input
          type="date"
          className="bg-transparent text-gray-700 text-xs outline-none w-full"
          value={value || ''}
          onChange={e => updateCell(row.id, col.id, e.target.value)}
        />
      );
    ...
    if (col.type === 'currency') {
      return (
        <div className="flex items-center gap-0.5">
          ...
          <input
            type="text"
            className="bg-transparent text-gray-700 text-sm outline-none w-full text-right"
            placeholder="0.00"
            value={value || ''}
            onChange={e => updateCell(row.id, col.id, e.target.value)}
          />
    ...
    return (
      <input
        className="bg-transparent text-gray-700 text-sm outline-none w-full placeholder-gray-300"
        value={value || ''}
  ...
  return (
    <div className="flex flex-col h-full bg-white text-gray-900">
      {/* ---- Toolbar ---- */}
      <div className="flex items-center justify-between px-5 py-3 border-b border-gray-200 bg-white relative z-40">
        {/* Left: Title */}
        <div className="flex items-center gap-3">
          ...
          {isEditingName ? (
            <input
              ref={nameInputRef}
              className="bg-gray-100 text-gray-900 text-base font-bold rounded-lg border border-indigo-400 px-3 py-1 outline-none"
            ...
            <h2
              className="text-base font-bold text-gray-800 cursor-pointer hover:text-indigo-600 transition-colors"
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button
            onClick={handleNew}
            className="px-3 py-2 rounded-lg text-xs font-medium text-gray-500 hover:text-gray-900 hover:bg-gray-100 transition-all flex items-center gap-1.5"
          ...
          <div className="relative" ref={loadMenuRef}>
            ...
            {isLoadMenuOpen && (
              <div className="absolute right-0 top-full mt-1 w-60 bg-white border border-gray-200 rounded-xl shadow-xl overflow-hidden z-50">
                <div className="py-1">
                  {savedTables.length === 0 ? (
                    <div className="px-4 py-3 text-xs text-gray-400 text-center">No saved tables</div>
                  ) : (
                    savedTables.map(table => (
                      <div key={table._id} className="relative group flex items-center">
                        <button
                          onClick={() => handleLoad(table._id)}
                          className="flex-1 text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors">
                          {table.name}
          ...
          <button
            ...
            className="px-3 py-2 rounded-lg text-xs font-semibold text-violet-600 hover:text-violet-800 hover:bg-violet-50 transition-all flex items-center gap-1.5">
            <SparklesIcon className="w-4 h-4" />
            ...
            {isCopyMenuOpen && (
              <div className="absolute right-0 top-full mt-1 w-52 bg-white border border-gray-200 rounded-xl shadow-xl overflow-hidden z-50">
                ...
                <button
                  ...
                  <span className="text-gray-500">Color Only</span>
                </button>
              </div>
            )}
          </div>
          ...
          <button
            ...
            className="px-3 py-2 rounded-lg text-xs font-medium text-gray-500 hover:text-red-600 hover:bg-red-50 transition-all flex items-center gap-1.5"
            title="Export to PDF">
          ...
          <button
            ...
            className="px-3 py-2 rounded-lg text-xs font-medium text-gray-500 hover:text-emerald-700 hover:bg-emerald-50 transition-all flex items-center gap-1.5"
      ...
      <div className="flex-1 overflow-auto relative">
        ...
        {/* Body */}
        <div className="min-w-full">{rows.map(row => renderRow(row))}</div>
        ...
        {rows.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 px-8 text-center">
            ...
            <div className="flex gap-2">
              <button
                onClick={addRow}
                className="px-4 py-2 rounded-lg bg-gray-100 text-gray-600 text-xs font-medium hover:bg-gray-200 transition-colors flex items-center gap-1.5">
                <PlusIcon className="w-4 h-4" />
                Add Row
              </button>
              <button
        ...
        {rows.length > 0 && (
          <button
            onClick={addRow}
            className="w-full py-2.5 text-xs text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 transition-all flex items-center justify-center gap-1.5 border-b border-gray-100">
            <PlusIcon className="w-3.5 h-3.5" />
            Add Row
          </button>
        )}
      ...
      {/* AI Modal */}
      <AISmartImportModal isOpen={isAIModalOpen} onClose={() => setIsAIModalOpen(false)} onImport={handleAIImport} />
