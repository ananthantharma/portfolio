  ...
  return (
    <div
      ...
      <div className="flex items-center gap-1.5 overflow-hidden flex-1 min-w-0">
        {column.id !== 'name' && (
          <div {...attributes} {...listeners} className="cursor-grab hover:text-zinc-300 text-zinc-500 shrink-0">
  ...
  const renderRow = (row: TableRow, depth: number = 0) => {
    ...
    return (
      <React.Fragment key={row.id}>
        <div
          className={`flex border-b border-white/[0.04] group transition-colors ${
            isGroup && depth === 0 ? 'bg-zinc-800/40 hover:bg-zinc-800/70' : 'bg-zinc-900/20 hover:bg-zinc-800/30'
          }`}>
          {columns.map((col, idx) => (
            <div
              key={col.id}
              className={`relative flex items-center px-3 py-2 text-sm border-r border-white/[0.04] last:border-r-0 ${
          ...
          <div className="relative" ref={copyMenuRef}>
            <button
              ...
              className={`px-3 py-2 rounded-lg text-xs font-medium transition-all flex items-center gap-1.5 ${
                copySuccess ? 'text-green-400 bg-green-500/10' : 'text-zinc-400 hover:text-zinc-100 hover:bg-zinc-800'
              }`}
      ...
      <div className="flex-1 overflow-auto relative">
        ...
        {rows.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 px-8 text-center">
            ...
            <h3 className="text-sm font-semibold text-zinc-400 mb-1">No data yet</h3>
            <p className="text-xs text-zinc-600 mb-4 max-w-xs">
              Add rows manually, or use <span className="text-violet-400 font-medium"> AI Import</span> to paste
