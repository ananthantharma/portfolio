    ...
    const handleExport = async () => {
        ...
        try {
            ...
            if (includeTitle) {
                ...
                doc.text(new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }), margin, startY + 12);
                startY += 18;
            }
        ...
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-xl bg-white rounded-2xl shadow-2xl ring-1 ring-black/8 overflow-hidden">
                ...
                <div className="p-5 space-y-5">
                    ...
                    {/* Orientation */}
                    <div>
                        <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 block">Page Orientation</label>
                        <div className="flex gap-2">
                            {(['portrait', 'landscape'] as const).map(o => (
                                <button
                                    key={o}
                                    onClick={() => setOrientation(o)}
                                    className={`flex-1 flex flex-col items-center gap-2 py-3 rounded-xl border-2 transition-all ${orientation === o ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200 hover:border-gray-300'
                                        }`}>
                                    {/* Paper icon */}
                                    <div className={`rounded border-2 ${orientation === o ? 'border-indigo-400' : 'border-gray-300'} ${o === 'portrait' ? 'w-8 h-11' : 'w-11 h-8'
                                    ...
                                    <span className={`text-xs font-medium capitalize ${orientation === o ? 'text-indigo-700' : 'text-gray-500'}`}>{o}</span>
                                </button>
                            ))}
                        </div>
                    </div>
                        ...
                        {[
                            ...
                            <label key={opt.label} className="flex items-center gap-2 cursor-pointer">
                                <div
                                    ...
                                    className={`w-8 h-4.5 rounded-full transition-colors relative ${opt.value ? 'bg-indigo-500' : 'bg-gray-300'}`}
                                    style={{ height: '18px', width: '32px' }}>
                                    <div className={`absolute top-0.5 w-3.5 h-3.5 rounded-full bg-white shadow transition-transform ${opt.value ? 'translate-x-[14px]' : 'translate-x-0.5'}`} />
                                </div>
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-2">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button onClick={normalize} className="text-[10px] px-2 py-0.5 bg-gray-100 hover:bg-gray-200 text-gray-600 rounded transition-colors">
                                    Auto-balance
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="flex h-3 rounded-full overflow-hidden mb-3 gap-px bg-gray-100">
                            {columns.map((col, i) => {
                                ...
                                return (
                                    <div
                                        key={col.id}
                                        style={{ width: `${colWidths[col.id] ?? 0}%`, backgroundColor: `hsl(${hues[i % hues.length]},70%,60%)` }}
                            ...
                            {columns.map((col, i) => {
                                ...
                                return (
                                    <div key={col.id} className="flex items-center gap-3">
                                        <div className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: `hsl(${hues[i % hues.length]},70%,60%)` }} />
                ...
                <div className="flex items-center gap-3 px-5 py-4 bg-gray-50 border-t border-gray-100">
                    <button onClick={onClose} className="flex-1 py-2 rounded-xl border border-gray-200 text-sm text-gray-600 hover:bg-gray-100 transition-colors font-medium">
                        Cancel
                    </button>
                    <button
