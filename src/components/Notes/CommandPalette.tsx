...
const CommandPalette: React.FC<CommandPaletteProps> = ({
    ...
    useEffect(() => {
        if (isOpen) {
            fetchItems('', false, false).then(pages => {
        ...
        const timer = setTimeout(async () => {
            if (!query.trim()) {
                ...
                const recent: CommandItem[] = recentPages.map(page => ({
                    ...
                    action: () => { onSelectTask(page); onClose(); },
                    category: 'recent',
                }));
            ...
            try {
                ...
                const mappedResults: CommandItem[] = noteResults.map(item => ({
                    ...
                    icon: (item as any).type === 'section'
                        ? <FolderIcon className="w-4 h-4" />
                        : <DocumentTextIcon className="w-4 h-4" />,
                    action: () => { onSelectTask(item); onClose(); },
                    category: 'result',
                }));
                        ...
                        <Dialog.Panel className="mx-auto max-w-xl transform overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-black/5 transition-all">
                                            ...
                                            <div key={catKey}>
                                                ...
                                                {items.map((item) => {
                                                    ...
                                                    return (
                                                        <li
                                                            key={item.id}
                                                            className={`cursor-default select-none mx-2 px-3 py-2.5 rounded-lg flex items-center justify-between transition-colors duration-75 ${isSelected ? 'bg-gray-900 text-white' : 'text-gray-700 hover:bg-gray-50'
                                                                }`}
                                                            onClick={() => item.action()}
                                                            onMouseEnter={() => setSelectedIndex(currentIndex)}
                                                        >
                                                            <div className="flex items-center gap-3 min-w-0">
                                                                <div className={`flex-shrink-0 p-1 rounded-md ${isSelected ? 'text-white' : 'text-gray-400'
                                                                    }`}>
                                                                    {item.icon}
                                                                </div>
                                                                <div className="flex flex-col min-w-0">
                                                                    <span className={`text-[13px] font-medium truncate ${isSelected ? 'text-white' : 'text-gray-800'
                                                                        }`}>
                                                                        {item.title}
                                                                    </span>
                                                                    {item.subtitle && (
                                                                        <span className={`text-[11px] truncate ${isSelected ? 'text-gray-300' : 'text-gray-400'
                                                                            }`}>
                                                                            {item.subtitle}
                                                                        </span>
                                                                    )}
                                                            ...
                                                            {item.shortcut && (
                                                                <kbd className={`text-[10px] font-mono px-1.5 py-0.5 rounded ${isSelected ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-400'
                                                                    }`}>
                                                                    {item.shortcut}
                                                                </kbd>
                                                            )}
                            ...
                            <div className="flex items-center justify-between border-t border-gray-100 px-4 py-2">
                                <div className="flex items-center gap-3 text-[10px] text-gray-300">
                                    <span className="flex items-center gap-1"><kbd className="font-mono bg-gray-100 px-1 py-0.5 rounded text-gray-400">↑↓</kbd> navigate</span>
                                    <span className="flex items-center gap-1"><kbd className="font-mono bg-gray-100 px-1 py-0.5 rounded text-gray-400">↵</kbd> select</span>
                                    <span className="flex items-center gap-1"><kbd className="font-mono bg-gray-100 px-1 py-0.5 rounded text-gray-400">esc</kbd> close</span>
                                </div>
                            </div>
                        </Dialog.Panel>
                    </Transition.Child>
