      ...
      <div className="relative w-full max-w-5xl h-full max-h-[85vh] bg-white rounded-3xl overflow-hidden shadow-2xl flex flex-col animate-in zoom-in-95 duration-300 border border-slate-100">
        ...
        <div className="flex items-center justify-between px-8 py-5 border-b border-gray-100 bg-white/80 backdrop-blur-md sticky top-0 z-10">
                  ...
                  <React.Fragment key={f.id}>
                    ...
                    <span
                      className={`text-[11px] font-bold cursor-pointer transition-colors ${
                        i === currentFolder.length - 1 ? 'text-indigo-600' : 'text-gray-400 hover:text-gray-600'
                      }`}
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={onClose}
              className="p-2.5 rounded-full hover:bg-gray-100 text-gray-400 transition-all hover:rotate-90">
              <X className="w-6 h-6" />
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto bg-slate-50/50 p-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
            {currentFolder.length > 1 && !loading && (
              <div
                onClick={goBack}
                className="group relative bg-white p-5 rounded-2xl border border-slate-200/60 shadow-sm hover:shadow-md hover:border-indigo-200 transition-all cursor-pointer flex flex-col items-center justify-center gap-3">
                ...
                <span className="text-xs font-bold text-slate-500 group-hover:text-indigo-600 antialiased">
                  Go Back
                </span>
              </div>
            )}
            ...
            {loading ? (
              Array.from({length: 12}).map((_, i) => (
                <div
                  key={i}
                  className="bg-white/40 p-5 rounded-2xl border border-slate-100 animate-pulse flex flex-col items-center gap-3">
              ...
              files.map(file => (
                <div
                  ...
                  className={`group relative bg-white p-5 rounded-2xl border border-slate-200/60 shadow-sm hover:shadow-xl hover:border-indigo-100 transition-all duration-300 cursor-pointer flex flex-col items-center gap-3 pt-8 ${
                    file.mimeType === 'application/vnd.google-apps.folder' ? 'hover:-translate-y-1' : ''
                  }`}>
                  {/* Icon */}
                  <div
                    className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-inner relative ${
                      file.mimeType === 'application/vnd.google-apps.folder' ? 'bg-amber-50' : 'bg-slate-50'
                    }`}>
                  ...
                  <div className="text-center w-full px-2">
                    <p
                      className="text-[12px] font-bold text-gray-700 truncate group-hover:text-indigo-600 transition-colors"
                      title={file.name}>
                      {file.name}
                    </p>
