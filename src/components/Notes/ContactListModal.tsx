                                ...
                                <Dialog.Panel className="w-full max-w-5xl transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all h-[85vh] flex flex-col">
                                    <Dialog.Title as="h3" className="text-lg font-medium leading-6 text-gray-900 flex justify-between items-center mb-4 flex-shrink-0 border-b pb-3">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <FilterDropdown label="Type" value={filterType} onChange={setFilterType} options={CONTACT_TYPES} />
                                            <FilterDropdown label="Department" value={filterDepartment} onChange={setFilterDepartment} options={CONTACT_DEPARTMENTS} />
                                            <FilterDropdown label="Position" value={filterPosition} onChange={setFilterPosition} options={CONTACT_POSITIONS} />
                                        ...
                                        {loading ? (
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                                {filteredContacts.map(contact => (
                                                    <div className="relative flex items-center space-x-3 rounded-lg border border-gray-300 bg-white px-4 py-3 shadow-sm focus-within:ring-2 focus-within:ring-indigo-500 focus-within:ring-offset-2 hover:border-gray-400 group" key={contact._id}>
                                                        ...
                                                        <div className="min-w-0 flex-1">
                                                            <div className="flex justify-between items-start">
                                                                ...
                                                                <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${contact.type === 'Internal' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'}`}>
                                                                    {contact.type}
                                                                </span>
                                                            </div>
