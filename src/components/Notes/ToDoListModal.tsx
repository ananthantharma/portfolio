  ...
  return (
    <div className="relative flex items-center gap-2">
      ...
      <div className="flex flex-col">
        <span className="text-[11px] font-semibold text-gray-700">{completed}/{total}</span>
...
const FilterPill = React.memo(({
  ...
  label: string; active: boolean; onClick: () => void; variant?: 'default' | 'danger' | 'warning' | 'success';
}) => {
  const colors = {
    default: active ? 'bg-gray-900 text-white shadow-sm' : 'bg-white text-gray-600 hover:bg-gray-50 border border-gray-200',
    danger: active ? 'bg-red-600 text-white shadow-sm' : 'bg-white text-red-600 hover:bg-red-50 border border-red-200',
    warning: active ? 'bg-amber-500 text-white shadow-sm' : 'bg-white text-amber-600 hover:bg-amber-50 border border-amber-200',
    success: active ? 'bg-emerald-600 text-white shadow-sm' : 'bg-white text-emerald-600 hover:bg-emerald-50 border border-emerald-200',
  };
...
const ToDoListModal: React.FC<ToDoListModalProps> = React.memo(({ isOpen, onClose, onNavigate }) => {
  ...
  const handleAIPrioritySuggest = async () => {
    ...
    try {
      const taskList = unprioritized.map(t =>
        `- "${t.title}" (due: ${new Date(t.dueDate).toLocaleDateString()}, category: ${t.category || 'None'})`
      ).join('\n');
      ...
      if (jsonMatch) {
        ...
        for (const suggestion of suggestions) {
          const matchingTodo = unprioritized.find(t =>
                  ...
                  <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 flex-shrink-0">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex bg-gray-100 rounded-lg p-0.5">
                        ...
                        <button
                          className={`p-1.5 rounded-md transition-all ${viewMode === 'board' ? 'bg-white shadow-sm text-gray-800' : 'text-gray-400 hover:text-gray-600'}`}
                  ...
                  {!showCompleted && (
                    <div className="px-6 py-2.5 flex items-center gap-4 border-b border-gray-50 flex-shrink-0">
                      ...
                      <div className="flex items-center gap-1.5">
                        <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mr-1">Priority</span>
                        <FilterPill label="All" active={filterPriority === 'All'} onClick={() => setFilterPriority('All')} />
                        <FilterPill label="High" active={filterPriority === 'High'} onClick={() => setFilterPriority('High')} variant="danger" />
                        <FilterPill label="Medium" active={filterPriority === 'Medium'} onClick={() => setFilterPriority('Medium')} variant="warning" />
                        <FilterPill label="Low" active={filterPriority === 'Low'} onClick={() => setFilterPriority('Low')} variant="success" />
                      </div>
                      ...
                      <div className="flex items-center gap-1.5 overflow-x-auto">
                        <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mr-1 flex-shrink-0">Category</span>
                        <FilterPill label="All" active={filterCategory === 'All'} onClick={() => setFilterCategory('All')} />
                        {CATEGORIES.map(cat => (
                          <FilterPill key={cat} label={cat.replace('!', '')} active={filterCategory === cat} onClick={() => setFilterCategory(cat)} />
                        ))}
                      </div>
                      ...
                      <div className="flex items-center gap-1.5 flex-shrink-0">
                        <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mr-1">Sort</span>
                        {(['priority', 'dueDate', 'title', 'category'] as SortField[]).map(field => (
                          <button
                            className={`px-2 py-1 text-[10px] font-medium rounded-md flex items-center gap-1 capitalize transition-colors ${sortField === field
                            ...
                            {sortField === field && (
                              sortDirection === 'asc' ? <ArrowUpIcon className="h-2.5 w-2.5" /> : <ArrowDownIcon className="h-2.5 w-2.5" />
                            )}
                          </button>
                        ))}
                      </div>
                            ...
                            <div
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2">
                                  ...
                                  <h4 className={`text-[13px] font-medium truncate ${todo.isCompleted ? 'text-gray-400 line-through' : 'text-gray-800'}`}>
                                    {todo.title}
                                  </h4>
                                  {todo.category && (
                                    <span className={`inline-block px-1.5 py-0.5 text-[10px] rounded-md font-medium ${getCategoryStyle(todo.category)}`}>
