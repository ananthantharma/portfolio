import {EyeIcon, EyeSlashIcon, LinkIcon, PencilIcon, TrashIcon} from '@heroicons/react/24/outline';
...
const PasswordItem: React.FC<PasswordItemProps> = React.memo(({password, onDelete, onUpdate}) => {
    ...
    return (
      <div className="mb-4 rounded-lg bg-gray-800 p-4 shadow-md border border-gray-700">
        ...
        <div className="mt-4 flex justify-end gap-2">
          <button className="rounded px-3 py-1 text-gray-400 hover:text-white" onClick={() => setIsEditing(false)}>
            Cancel
          </button>
          <button className="rounded bg-orange-500 px-3 py-1 text-white hover:bg-orange-600" onClick={handleSave}>
            Save
          </button>
        </div>
      </div>
  ...
  return (
    <div className="mb-4 flex flex-col justify-between rounded-lg bg-gray-800 p-4 shadow-md border border-gray-700 md:flex-row md:items-start">
      <div className="flex-1 space-y-2">
        ...
        {password.notes && (
          <p className="text-sm text-gray-400 italic border-l-2 border-gray-600 pl-2">{password.notes}</p>
        )}
      </div>
      ...
      <div className="mt-4 flex items-center gap-4 md:mt-0 md:ml-4">
        {password.password && (
          <>
            ...
            <button className="text-sm text-blue-400 hover:text-blue-300" onClick={handleCopy}>
              Copy
            </button>
          </>
        )}
        <button className="text-gray-400 hover:text-white" onClick={() => setIsEditing(true)}>
          <PencilIcon className="h-5 w-5" />
        </button>
        <button className="text-red-400 hover:text-red-300" onClick={() => onDelete(password._id)}>
